...
async function load() {
  ...
  try {
    ...
    // Раньше эта ветка молча проглатывала ошибки рендера (loading уже скрыт
    // к этому моменту) — баг с графиком базиса из-за этого было не видно ни
    // на экране, ни в консоли. Теперь дублируем в консоль и возвращаем
    // индикатор, чтобы будущие похожие ошибки не терялись бесследно.
    console.error('Ошибка при загрузке/рендере экрана анализа:', e)
    loading.style.display = 'block'
